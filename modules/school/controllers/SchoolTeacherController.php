...
use app\models\search\SchoolTeacherSearch;
...
/**
 * SchoolTeacherController implements the CRUD actions for SchoolTeacher model.
 */
class SchoolTeacherController extends Controller
{
    ...
    /**
     * Lists all SchoolTeacher models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new SchoolTeacherSearch();
    ...
    /**
     * Displays a single SchoolTeacher model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new SchoolTeacher model.
    ...
    { 
        ...
        if ( $user->create()) {
            ...
            $employeeRole = $auth->getRole(\app\helpers\Configuration::USER_ROLE_TEACHER);
            if($employeeRole){
                try
                {
    ...
    public function actionActivateSchoolTeacher($id)
    {
        if($id)
        {
            $model = $this->findModel($id);
            ...
            if($model->save(false))
            {
                \Yii::$app->getSession()->setFlash('success', yii::t('app', 'School teacher activated successfully....'));
                return $this->redirect(Yii::$app->request->referrer);
            }
        }
        \Yii::$app->getSession()->setFlash('danger', yii::t('app', 'School teacher not activated....'));
        return $this->redirect(Yii::$app->request->referrer);
    }
    public function actionDeactivateSchoolTeacher($id)
    {  
        if($id)
        {
            $model = $this->findModel($id);
            ...
            if($model->save(false))
            {
                \Yii::$app->getSession()->setFlash('success', yii::t('app', 'School teacher deactivated successfully....'));
                return $this->redirect(Yii::$app->request->referrer);
            }
        }
        \Yii::$app->getSession()->setFlash('danger', yii::t('app', 'School teacher not deactivated....'));
        return $this->redirect(Yii::$app->request->referrer);
    }
    ...
    {
        if (($model = SchoolTeacher::find()->where(['id' => $id])->andWhere(['school_id' => \Yii::$app->getRequest()->getCookies()->getValue('schoolId')])->one()) !== null) {
            return $model;
        }
