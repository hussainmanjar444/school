...
class StudentController extends Controller
{
    ...
    public function createUser(&$model)
    {
        ...
        if ( $user->create()) {
            ...
            $employeeRole = $auth->getRole(\app\helpers\Configuration::USER_ROLE_STUDENT);
            if($employeeRole){
                try
                {
    ...
    public function actionActivateStudent($id)
    {
        if($id)
        {
            $model = $this->findModel($id);
            ...
            if($model->save(false))
            {
                \Yii::$app->getSession()->setFlash('success', yii::t('app', 'Student activated successfully....'));
                return $this->redirect(Yii::$app->request->referrer);
            }
        }
        \Yii::$app->getSession()->setFlash('danger', yii::t('app', 'Student not activated....'));
        return $this->redirect(Yii::$app->request->referrer);
    }
    public function actionDeactivateStudent($id)
    {  
        if($id)
        {
            $model = $this->findModel($id);
            ...
            if($model->save(false))
            {
                \Yii::$app->getSession()->setFlash('success', yii::t('app', 'Student deactivated successfully....'));
                return $this->redirect(Yii::$app->request->referrer);
            }
        }
        \Yii::$app->getSession()->setFlash('danger', yii::t('app', 'Student not deactivated....'));
        return $this->redirect(Yii::$app->request->referrer);
    }
