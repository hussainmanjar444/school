...
class InventoryIssueController extends Controller
{
    ...
    { 
        ...
        if($model->load(Yii::$app->request->post()))
         { 
            foreach ($model->inventory_id as $inventory_id) {
                $modelNew = new InventoryIssue();
                $modelNew->scenario = $model::SCENARIO_NEW_BOOK_ISSUED;
                $modelNew->student_id = $id;
                $modelNew->issued_date = date("Y-m-d");
                $modelNew->inventory_id = $inventory_id;
                $modelNew->issue_tilldate = date("Y-m-d", strtotime("+".Configuration::LIBRARY_MAX_ISSUE_TILL_DAYS." days"));
                $modelNew->issue_by = Yii::$app->user->identity->id;
                $modelNew->status = Configuration::ISSUED;
                if($modelNew->save())
                { 
                    $modelInventory = Inventory::findOne($inventory_id);
                    $modelInventory->avaliable_quantity -= 1;
                    $modelInventory->save(false);  
                }
            }
